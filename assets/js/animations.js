...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // ===== 15. SAFETY FALLBACK FOR PREVIEW IFRAMES (Bolt IA, Arena, Lovable) =====
  // If IntersectionObserver/ScrollTrigger don't fire in iframe, force reveal after timeout
  function gobexSafetyReveal(){
    const selectors = [
      '.feature-item',
      '.service-item',
      '.testimonial-item',
    ...
    selectors.forEach(sel=>{
      document.querySelectorAll(sel).forEach(el=>{
        ...
        if(cs.opacity === '0' || el.style.opacity === '0'){
          el.style.opacity='1';
          // Reset transform to visible state
          if(sel === '.product-card' || sel === '.feature-item' || sel === '.service-item'){
            el.style.transform='translateY(0) scale(1)';
          } else if(sel.includes('data-reveal') || sel.includes('data-aos')){
            el.style.transform='translateY(0) translateX(0) scale(1)';
            el.classList.add('aos-animate','revealed','safety-revealed');
          } else {
            el.style.transform='translateY(0) translateX(0) scale(1)';
          }
          el.classList.add('safety-revealed');
        }
      });
    });
    // Ensure banner texts visible
    document.querySelectorAll('.banner h1, .banner p, .banner .btn, .banner-stats').forEach(el=>{
      if(getComputedStyle(el).opacity === '0') el.style.opacity='1';
    });
  }

  // Normal browsers: reveal after 2.5s if observers failed
  setTimeout(gobexSafetyReveal, 2500);
  // Iframe preview (Bolt IA): faster reveal 800ms
  try{
    if(window.self !== window.top){
      console.log('[GOBEX] Iframe preview detected - fast safety reveal');
      setTimeout(gobexSafetyReveal, 800);
    }
    // Accessibility: reduced motion
    if(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches){
      setTimeout(gobexSafetyReveal, 300);
    }
  }catch(e){
    // Cross-origin iframe may throw - still apply safety
    setTimeout(gobexSafetyReveal, 1000);
  }
  // Extra safety on full page load
  window.addEventListener('load', ()=> setTimeout(gobexSafetyReveal, 1200));
  // And if AOS is present, ensure it doesn't block
  if(window.AOS){
    setTimeout(()=>{
  ...
  console.log('%c GOBEX Animations Loaded ✨ + Safety Fallback ', 'background:#0A2F5E;color:#FFB81C;padding:8px 16px;border-radius:8px;font-weight:700;');
});
